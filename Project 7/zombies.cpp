...
void Player::moveOrAttack(int dir)
{
    ...
    int adjacentRow = m_row;
    int adjacentCol = m_col;
    m_arena->determineNewPosition(adjacentRow, adjacentCol, dir);

    if (m_arena->numZombiesAt(adjacentRow, adjacentCol) > 0)
    {
        m_arena->attackZombieAt(adjacentRow, adjacentCol, dir);
        return;
    }
...
void Arena::display() const
{
    ...
    for (int i = 0; i < m_nZombies; i++)
    {
        char &gridChar = grid[m_zombies[i]->row() - 1][m_zombies[i]->col() - 1];
        if (gridChar == '.')
            gridChar = 'Z';
        else if (gridChar == 'Z')
            gridChar = '2';
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    for (int i = 0; i < m_nZombies; i++)
    {
		Zombie* &z = m_zombies[i];
		if (z->row() == r && z->col() == c)
		{
			if (z->getAttacked(dir))
			{
				delete z;
				m_nZombies--;
				z = m_zombies[m_nZombies];
				return true;
			}
            return false;
        }
